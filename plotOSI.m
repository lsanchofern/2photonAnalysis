...
for i=1:length(spks)
    osi_contra(i)=1-SigCells.ContraCell(i).cv_ori; 
end
%ipsilateral cells monocular 
for i=1:length(SigCells.IpsiCell)
    osi_ipsi(i)=1-SigCells.IpsiCell(i).cv_ori; 
end
%Binocular- contralateral
for i=1:length(SigCells.BinocCell)
    osi_binoc_contra(i)=1-SigCells.BinocCell(i).cv_ori.contra; 
end
%Binocular- ipsilateral  
for i=1:length(SigCells.BinocCell)
    osi_binoc_ipsi(i)=1-SigCells.BinocCell(i).cv_ori.ipsi; 
end
...
hold on 
swarmchart (x4, avg_osi_binoc_ipsi, 100, c4, 'filled', 'MarkerEdgeColor','black');
ylim([0 1]); 
title('Orientation Selectivity Index')
hold off 

name1=strcat(anim, '_OSI');
saveas(figOSI,fullfile(SaveDir1,name1), 'pdf');
saveas(figOSI,fullfile(SaveDir1,name1), 'tif');
